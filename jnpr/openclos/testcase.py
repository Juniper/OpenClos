...
def get_match (needle, cmds):
	...
	for haystack_orig in cmds:
		...
		if len(haystack)<len(needle):
			...
			match_macros = re.search(needle,haystack)
		...
		# Compare and complete macros case
			...
			for macro_name in macro_dict.keys():
				try:
					...
					macro_start_pos = haystack.find(macro_name)
		...
		# Find point of match and return remaining command case
			...
			index_of_diff = 0
			for char_a, char_b in zip(list(haystack_orig), list(needle)):
				if char_a!=char_b:
					break
				index_of_diff=index_of_diff+1
			if index_of_diff!=0:
				balance_haystack = haystack_orig[index_of_diff:]
				if balance_haystack!="":
					for macro_name in macro_dict.keys():
						if macro_name in balance_haystack.partition("_")[0]:
							balance_haystack=balance_haystack.partition("_")[2]
